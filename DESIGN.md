...
| 层级 | 类别 | 示例 | 解锁条件 |
|---|---|---|---|
| L1 | Navigation | 工作区切换、焦点切换 | 默认开放 |
| L2 | Window Operations | 浮动、关闭、移动、尺寸 | L1 中至少 70% 的键位达到 Mastered |
| L3 | Launchers | 终端、浏览器、截图 | L2 中至少 70% 的键位达到 Mastered |
| L4 | Groups | 编组、组内切页 | L3 中至少 70% 的键位达到 Mastered |
| L5 | Scratchpad | 呼出、收起、移动到暂存区 | L4 中至少 70% 的键位达到 Mastered |

单个键位的 **Mastered** 定义：最近 10 次有效计时卡尝试中，
至少 8 次正确，且正确样本的 P75 反应时间不超过该层目标值。错误和超时计入
正确率，但不伪造一个反应时间；解锁依据正确率与分位数，不使用容易被极端值
影响的平均反应时间。

解锁只开放新内容，不阻止玩家继续玩旧层，也不因后续状态下降而重新上锁。
