...
状态：设计草案 v0.5　·　对应插件版本 0.2.0　·　持续掌握循环已定稿、实现待开发
...
文档中的 0.2.0 表示本轮产品设计和插件发布版本。标记为“目标”的能力只有在源码、
测试和安装包同步完成后才算已实现；单独更新本文档不会改变当前游戏行为。
...
- 不模拟一个假桌面；Keycade 的差异化是“长在真实会话里”，但对局
  中仍然只识别按键，不执行危险动作。
- 不采集全局键盘输入，不做通用键盘记录器。
- 0.2.0 不做 bar widget、通知、徽章和复杂主题动画；只加入对学习有用的最小音效。
...
5. 调度器从持久化队列生成 24 张混合牌组，分为 3 波，每波 8 张；到期项、未学
   项、薄弱项和维护项可以出现在同一波，波次之间暂停 2 秒并展示学习进度。
6. 玩家按出卡片对应组合键：
   - **引导卡按对**：立即继续；该键位在本局后 3–5 张卡内转为学习卡。
...
当类别数量超过单波卡片数时逐波轮换起点。分类均衡是同优先级题目之间的次级
...
到期的 Mastered 项永远不能挤掉未学项或到期项。单个快捷键在每波最多出现两次，
且不连续重复。因首次错误产生的纠错跟按不占 24 张计划卡配额；该项目还会插入
本局后 3–5 张卡的位置再次进行无提示回忆。
...
```jsonc
{
  ...
  "firstTryAttempts": 7,
  "firstTryCorrect": 5,
  "recentFirstTry": [true, false, true, true, true, true],
...
达到 Mastered 至少需要：完成过引导；跨至少 3 局累计 5 次无提示首次正确；最近
6 次首次作答至少 5 次正确。反应时间用于判断是否流畅和设置舒适倒计时，不单独
决定 Mastered。键位配置变化时，新增项进入 Unseen，删除或被用户排除的项标记
...
音效服务于节奏和记忆确认，不承载任何唯一信息。0.2.0 内置本地短音效，不联网、
不依赖系统主题，也不在按键路径中启动外部播放器：

| 事件 | 音效设计 | 规则 |
|---|---|---|
| **首次正确** | 约 80–120ms、轻快上行音 | 输入确认后立即播放，不阻塞下一卡 |
| **首次错误/超时** | 约 120–180ms、柔和低音 | 每张卡只播放一次，纠错期间连续按错不反复轰炸 |
| **纠错完成** | 比首次正确更轻的确认音 | 表示可以继续，但不制造“抵消错误”的错觉 |
...
预加载；设置提供总开关、倒计时音效开关和音量，默认开启、音量 60%，并尊重
...
├── stats.json         # 每个稳定 chord id 的首次作答、反应时间和累计统计
...
- 倒计时使用分段能量槽，不使用平滑圆环；顶部 HUD 展示“本局首次正确率”、
  “新学会”“已掌握/总数”和“待复习”，不展示生命、分数或失败状态。
...
| 错误键位被重复轰炸 | 每波最多两次、不连续重复；纠错音只播放一次；短间隔重测插在 3–5 卡后 |
...
- 以固定种子模拟足够多局后，所有未排除项都被 `coverageCursor` 覆盖，且到期项
  不会被未到期 Mastered 项挤出；
- 错误只记录一次，正确答案保持显示直到输入正确，并在 3–5 卡后再次无提示出现；
- 正确、错误、纠错完成和倒计时音效触发准确；关闭声音后没有播放或计时异常；
...
- 0.2.0 不接通知、不做 bar；加入最小本地音效并提供独立开关。
...
- 0.2.0 音效采用代码合成还是明确许可证的 CC0 素材；两者都必须随包离线分发。
