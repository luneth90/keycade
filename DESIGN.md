...
状态：设计版本 v0.5.5　·　对应插件版本 0.2.5　·　总掌握进度与分层反应时间已实现
...
文档中的 0.2.5 表示本轮产品设计和插件发布版本。源码、测试和安装包必须保持
...
6 次首次作答至少 5 次正确。普通卡首次答对时显示本次反应时间；结算页显示本局
P75；每个快捷键只持久化最近 10 次有效反应时间，不在主界面罗列全部原始样本。
