...
| 规则 | 是否触及 | 结论与依据 |
| --- | --- | --- |
| R1 不执行用户配置 | 触及 | **守住**。`kb_file` 与用户 XKB 覆盖目录一律降级，不读取任何用户可写内容；新增的 `hyprctl getoption` 已申报并写入 §01 边界清单 |
| R2 双端有界 | 触及 | **守住**。新增表设 4 KiB / 256 项 / 每项 16 码上限；QML 侧独立复验类型、长度与数值范围，违规丢表而非整体失败 |
