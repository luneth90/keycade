...
//
// This deliberately small vocabulary is pinned by
// tests/fixtures/text-keys.js so producer and consumer cannot drift silently.
...
// "S-" is Shift wherever a key is written this way - Vim's `<S-Tab>`, tmux's
// `S-Up` - and it is Shift in every collector on the Python side too. It used
// to mean Super here alone, which would have taught Super for a prefix the
// machine had set to Shift. Super is spelled out.
var specModifiers = {
...
function parseKeySpec(value) {
  ...
  if (!body.length) return null
  // Shift is not a modifier of a character in this judging mode, it is inside
  // the character: "S-x" is the key that types "X". On a named key it has no
  // character to fold into and stays a modifier, which is the same rule every
  // collector applies. Tab is the exception both sides refuse: Qt reports
  // Backtab and Tab as one key, so the two could not be told apart.
  var shifted = Boolean(mods & SHIFT)
  mods = mods & ~SHIFT
  // "<Space>" and "Space" both name the key, and so does a bare " ".
  var bare = body.replace(/^<|>$/g, "")
  var named = canonicalName(bare)
  if (named) {
    if (shifted && named === "TAB") return null
    return normalizedStep({ mods: shifted ? mods | SHIFT : mods, named: named })
  }
  var character = namedCharacter(bare)
  if (character) {
    if (shifted && character.toUpperCase() === character.toLowerCase()) return null
    return normalizedStep({ mods: mods, text: shifted ? character.toUpperCase() : character })
  }
  if (bare.length !== 1) return null
  if (shifted) {
    if (bare.toUpperCase() === bare.toLowerCase()) return null
    return normalizedStep({ mods: mods, text: bare.toUpperCase() })
  }
...
// One spelling for a left-hand side, so a table entry and a user's own line
// meet. Vim reads the name inside <> without regard to case - <Leader>, <CR>,
// <C-X> are the same keys as <leader>, <cr>, <C-x> - while the characters
// outside them are decisive, which is why <leader>ff and <leader>fF are two
// different mappings. Lower-casing only what is inside the brackets is
// therefore exactly the equivalence Vim itself applies. Without it a config
// written <Leader>ff added a second card next to the packaged <leader>ff
// instead of replacing its description.
function canonicalNotation(value) {
  var text = String(value === undefined || value === null ? "" : value)
  return text.replace(/<[^<>]*>/g, function(group) { return group.toLowerCase() })
}
