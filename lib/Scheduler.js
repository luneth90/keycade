...
function updateCoverageCursor(bindings, stats, selectedUnseen) {
  if (!bindings.length || !selectedUnseen.length) return
  var ordered = stableBindings(bindings)
  var selected = {}
  selectedUnseen.forEach(function(binding) { selected[binding.id] = true })
  var cursor = Number(stats.coverageCursor || 0) % ordered.length
  var remaining = selectedUnseen.length
  for (var step = 0; step < ordered.length && remaining > 0; step++) {
    var index = (cursor + step) % ordered.length
    if (selected[ordered[index].id]) remaining -= 1
...
function build(bindings, stats, cardCount, options) {
  ...
  var unseenTaken = takeUnique(deck, pools.unseen, 6, "unseen", stats)
  updateCoverageCursor(bindings, stats, pools.unseen.slice(0, unseenTaken))
