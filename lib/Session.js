...
// A run belongs to the training ground it was played on: resuming into a
// different one would mix two decks and two run counters.
function canResume(session, runId, bindings, cardLimit, profileId) {
  ...
  if (String(session.profileId || "") !== String(profileId || "")) return false
...
// them, and everything stored back then was played on the default ground.
// They are qualified on the way in, so a run interrupted before the upgrade
...
function sanitize(value) {
  ...
  var legacy = !Profiles.valid(value.profileId)
  var profileId = legacy ? Profiles.defaultId() : String(value.profileId)
  var result = {
    schemaVersion: 1,
    profileId: profileId,
