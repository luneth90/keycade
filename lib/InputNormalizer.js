...
function matches(binding, input, options) {
  ...
  // With a reproduced keymap the accepted keycodes are the answer, absence
  // included: Hyprland resolves the pressed keycode to its base-level keysym
  // and compares keysyms, so the character this key happens to produce under
  // the held modifiers says nothing about whether the bind would fire.
  ...
  // No keymap, or an event carrying no keycode at all, as a virtual keyboard
  // can produce. Fall back to comparing the character.
