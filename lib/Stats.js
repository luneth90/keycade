...
function masteryReady(item) {
  var recent = item.recentFirstTry.slice(-3)
  return item.guidedCompleted
      && item.firstTryCorrect >= 3
...
function migrate(value) {
  ...
  migrated.totalTrainingMs = Math.max(0, Number(value.totalTrainingMs || 0))
  migrated.firstMasteryAt = Math.max(0, Number(value.firstMasteryAt || 0))
  migrated.firstMasteryRun = Math.max(0, Number(value.firstMasteryRun || 0))
  migrated.firstMasteryCelebrated = Boolean(value.firstMasteryCelebrated)
  Object.keys(value.bindings).forEach(function(id) {
...
function addTrainingTime(stats, elapsedMs) {
  ...
  stats.totalTrainingMs = Math.max(0, Number(stats.totalTrainingMs || 0)) + elapsed
