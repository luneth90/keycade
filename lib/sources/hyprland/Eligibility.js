// The Hyprland training ground's own rules for what can be trained. Nothing
// here is a global rule: another ground excludes different things, for
// different reasons, and says so in its own directory.
.pragma library
...
function reason(binding, options) {
  ...
  // Esc is how a run is saved and left. A bind that answers with a bare Esc
  // cannot be answered at all today - releasing it exits instead - so it is
  // dealt as a card nobody can clear. Esc held with a modifier is a different
  // gesture, decided on release, and stays trainable.
  if (Normalizer.canonicalKey(binding.key) === "ESCAPE" && !Number(binding.modMask || 0))
    return "escape-in-answer"
...
function filter(bindings, options) {
  var opts = options || {}
  var profileId = Profiles.valid(opts.profile) ? String(opts.profile) : Profiles.defaultId()
  ...
  var candidates = []
  var signatures = Session.safeMap()
  for (var i = 0; i < bindings.length; i++) {
    var binding = bindings[i]
    if (reason(binding, opts)) continue
    var chord = Normalizer.chordId(binding, opts)
    if (!signatures[chord]) signatures[chord] = Session.safeMap()
  ...
  for (var j = 0; j < candidates.length; j++) {
    ...
    // The local id is this source's own naming and stays byte for byte what
    // it has always been: it is what every stored exclusion already names.
    // The qualified id is what the layers above use, so two training grounds
    // cannot collide in stats or in a deck.
    item.localId = Normalizer.bindingId(item)
    item.id = Profiles.qualify(profileId, item.localId)
    // One chord, which is the length-1 case of a sequence. Everything above
    // this line judges and draws answers, not binds.
    item.answer = AnswerMatcher.chordAnswer(item)
